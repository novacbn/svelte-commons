...
var DEPRECATE_READABLE = false;
var DEPRECATE_WRITABLE = false;
/**
...
function immutable(store, clone, start) {
    ...
    if (stores_1.is_writable(store))
        return overlay_1.overlay(store, clone, clone);
    return store_1.derived(store, clone);
}
exports.immutable = immutable;
// DEPRECATED: The following functions were deprecated, and are now aliases
function immutable_readable(store, clone, start) {
    if (!DEPRECATE_READABLE) {
        console.warn("[svelte-commons] DEPRECATE: `immutable_readable` is deprecated, use `immutable` directly");
        DEPRECATE_READABLE = true;
    }
    if (!stores_1.is_readable(store) && start)
        store = store_1.readable(store, start);
    return immutable(store, clone, start);
}
exports.immutable_readable = immutable_readable;
function immutable_writable(store, clone, start) {
    if (!DEPRECATE_WRITABLE) {
        console.warn("[svelte-commons] DEPRECATE: `immutable_writable` is deprecated, use `immutable` directly");
